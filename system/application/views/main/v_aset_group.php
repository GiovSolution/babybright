...
var group_aset_lokasiField;
var group_aset_kelasField;
var group_aset_tanggalField;
...
Ext.onReady(function(){
	...
	function group_aset_create(){
		...
		var group_aset_lokasi_create=null;
		var group_aset_kelas_create=null;
		var group_aset_tanggal_create=null;
		...
		if(group_aset_lokasiField.getValue()!== null){group_aset_lokasi_create = group_aset_lokasiField.getValue();}
		if(group_aset_kelasField.getValue()!== null){group_aset_kelas_create = group_aset_kelasField.getValue();}
		if(group_aset_tanggalField.getValue()!== ""){group_aset_tanggal_create = group_aset_tanggalField.getValue().format('Y-m-d');}
		...
		if(group_aset_aktifField.getValue()!== null){group_aset_aktif_create = group_aset_aktifField.getValue();}
			...
			Ext.Ajax.request({  
				...
				params: {
					...
					group_aset_lokasi		: group_aset_lokasi_create,	
					group_aset_kelas		: group_aset_kelas_create,	
					group_aset_tanggal		: group_aset_tanggal_create,
	...
	function group_aset_reset_form(){
		...
		group_aset_lokasiField.reset();
		group_aset_lokasiField.setValue(null);
		...
		group_aset_tanggalField.reset();
		group_aset_tanggalField.setValue(null);
	...
	function group_aset_set_form(){
		...
		group_aset_lokasiField.setValue(group_asetListEditorGrid.getSelectionModel().getSelected().get('aset_group_lokasi'));
		group_aset_kelasField.setValue(group_asetListEditorGrid.getSelectionModel().getSelected().get('aset_group_kelas'));
		group_aset_tanggalField.setValue(group_asetListEditorGrid.getSelectionModel().getSelected().get('aset_group_tanggal'));
	...
	group_aset_DataStore = new Ext.data.Store({
		...
		reader: new Ext.data.JsonReader({
			...
			{name: 'aset_group_lokasi', type: 'string', mapping: 'aset_grup_lokasi'},
			{name: 'aset_group_kelas', type: 'string', mapping: 'aset_grup_kelas'},
			{name: 'aset_group_tanggal', type: 'date', dateFormat: 'Y-m-d', mapping: 'aset_grup_tanggal'},
	...
	group_aset_ColumnModel = new Ext.grid.ColumnModel(
		...
		{
			header: '<div align="center">' + 'Kode Aset' + '</div>',
			dataIndex: 'aset_group_kode',
			width: 90,
			sortable: true
		},
		{
			header: '<div align="center">' + 'Nama Aset' + '</div>',
			dataIndex: 'aset_group_nama',
			width: 100,
			sortable: true
		},
		{
			header: '<div align="center">' + 'Lokasi Aset' + '</div>',
			dataIndex: 'aset_group_lokasi',
			width: 100,
			sortable: true
		},
		{
		...
		{
			header: '<div align="center">' + 'Tanggal Beli' + '</div>',
			dataIndex: 'aset_group_tanggal',
			width: 200,
			sortable: true,
	...
	group_aset_kodeField= new Ext.form.TextField({
		...
		fieldLabel: 'Kode Aset <span style="color: #ec0000">*</span>',
		maxLength: 4,
		allowBlank: false,
		anchor: '50%'
	});
	...
	group_aset_namaField= new Ext.form.TextField({
		...
		fieldLabel: 'Nama Aset',
		maxLength: 250,
		anchor: '95%'
	});
	/* Identify  group_aset_lokasiField Field */
	group_aset_lokasiField= new Ext.form.TextField({
		id: 'group_aset_lokasiField',
		fieldLabel: 'Lokasi Aset',
		maxLength: 250,
		anchor: '95%'
	});
	...
	group_aset_usiaField= new Ext.form.TextField({
		...
		fieldLabel: 'Usia Penyusutan (Hari)',
		maxLength: 10,
		allowBlank: true,
		anchor: '50%'
	});
	...
	group_aset_tanggalField= new Ext.form.DateField({
		id: 'group_aset_tanggalField',
	...
	group_aset_createForm = new Ext.FormPanel({
		...
		items: [{
			...
			items:[
			{
				...
				layout: 'form',
				border:false,
				items: [group_aset_kodeField, group_aset_namaField, group_aset_lokasiField, group_aset_kelasField, group_aset_tanggalField, group_aset_usiaField, group_aset_keteranganField, group_aset_aktifField] 
			}
			]
		}]
		,
	...
	group_aset_searchWindow = new Ext.Window({
		...
		y: 0,
		modal: true,
		renderTo: 'elwindow_group_aset_search',
	...
	function display_form_search_window(){
		if(!group_aset_searchWindow.isVisible()){
			gudang_reset_SearchForm();
			group_aset_searchWindow.show();
		} else {
			group_aset_searchWindow.toFront();
		}
	}
  	/* End Function */
...
<div>
	<div class="col">
        ...
        <div id="elwindow_group_aset_search"></div>
    </div>
</div>
</body>