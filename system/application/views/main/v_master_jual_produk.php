<?
/* 	
	GIOV Solution - Keep IT Simple YEahhh!!!
*/
?>
...
*/
	...
	function master_jual_produk_create(){
		...
		// if(jproduk_bayarField.getValue()>=0 && jproduk_bayarField.getValue()<=jproduk_totalField.getValue()){
			if((is_master_jual_produk_form_valid() && dproduk_produk_id=="ada") &&
			  (jproduk_post2db=="CREATE" || jproduk_post2db=="UPDATE") && (jproduk_stat_dokField.getValue()=='Terbuka')){
				...
				// Customer Baru 
				var jproduk_cust_namaBaru_create="";
	            var jproduk_cust_telpBaru_create="";
	            var jproduk_cust_hpBaru_create="";
	            var jproduk_cust_keteranganBaru_create="";
				...
				*/
				...
				//Customer Baru Value :
				if(jproduk_cust_namaBaruField.getValue()!== null){jproduk_cust_namaBaru_create = jproduk_cust_namaBaruField.getValue();}  
	            if(jproduk_cust_telpBaruField.getValue()!== null){jproduk_cust_telpBaru_create = jproduk_cust_telpBaruField.getValue();}
	            if(jproduk_cust_hpBaruField.getValue()!== null){jproduk_cust_hpBaru_create = jproduk_cust_hpBaruField.getValue();}
	            if(jproduk_cust_keteranganBaruField.getValue()!== null){jproduk_cust_keteranganBaru_create = jproduk_cust_keteranganBaruField.getValue();}
						...
						if(i==dcount){
							...
							Ext.Ajax.request({
								...
								params: {
									...
									//Customer Baru Posting 
									jproduk_cust_nama_baru	: jproduk_cust_namaBaru_create,
						            jproduk_cust_telp_baru	: jproduk_cust_telpBaru_create,
						            jproduk_cust_hp_baru	: jproduk_cust_hpBaru_create,
						            jproduk_cust_keterangan_baru : jproduk_cust_keteranganBaru_create,
...
/*
	...
	function master_jual_produk_reset_form(){
		...
		jproduk_kembalianLabel.reset();
		jproduk_kembalianLabel.setValue(null);
 	...
 	function master_jual_produk_custBaruGroup_reset(){
 		jproduk_custBaruGroup.collapse(true);
 		jproduk_cust_namaBaruField.reset();
 		jproduk_cust_namaBaruField.setValue(null);
 		jproduk_cust_telpBaruField.reset();
 		jproduk_cust_telpBaruField.setValue(null);
 		jproduk_cust_hpBaruField.reset();
 		jproduk_cust_hpBaruField.setValue(null);
 		jproduk_cust_keteranganBaruField.reset();
 		jproduk_cust_keteranganBaruField.setValue(null);
 	}
					...
					callback: function(opts, success, response)  {
						 if (success) {
							if(memberDataStore.getCount()){
								...
								jproduk_cust_priorityField.setValue(jproduk_member_record.cust_priority_star);
	...
	function display_form_window(){	
		...
		jproduk_cust_priorityLabel.setText('');
		master_jual_produk_createWindow.hide();
		master_jual_produk_custBaruGroup_reset();
	}
  	/* End of Function */
	...
	jproduk_valid_memberLabel=new Ext.form.Label({ html: '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Member Valid: &nbsp;&nbsp;'});	
	...
	jproduk_cust_namaBaruField=new Ext.form.TextField({
		id: 'jproduk_cust_namaBaruField',
	...
	jproduk_cust_telpBaruField=new Ext.form.TextField({
		id: 'jproduk_cust_telpBaruField',
		fieldLabel: 'Telp Rmh',
		maxLength: 30,
		enableKeyEvents: true,
		maskRe: /([0-9]+)$/,
	...
	jproduk_cust_hpBaruField=new Ext.form.TextField({
		id: 'jproduk_cust_hpBaruField',
		fieldLabel: 'No HP',
		maxLength: 30,
		enableKeyEvents: true,
		maskRe: /([0-9]+)$/,
	...
	jproduk_cust_keteranganBaruField= new Ext.form.TextArea({
		id: 'jproduk_cust_keteranganBaruField',
		fieldLabel: 'Keterangan',
		maxLength: 250,
		anchor: '95%'
	});

	jproduk_custBaruGroup = new Ext.form.FieldSet({
		title: 'Customer Baru',
		checkboxToggle:true,
		autoHeight: true,
		layout:'column',
		...
		items:[
			{
				...
				layout: 'form',
				border:false,
				items: [jproduk_cust_namaBaruField, jproduk_cust_telpBaruField, jproduk_cust_hpBaruField] 
			},
			{
				columnWidth:0.5,
				layout: 'form',
				border:false,
				items: [jproduk_cust_keteranganBaruField] 
			} ]
	...
	jproduk_kembalianField= new Ext.form.NumberField({
	...
	jproduk_kembalianLabel= new Ext.form.DisplayField({
	...
	master_jual_produk_bayarGroup = new Ext.form.FieldSet({
		...
		items:[
			...
			,{
			  ...
			  /*jproduk_ket_diskField,*/ {xtype: 'spacer',height:10},/*jproduk_total_cfField,*/jproduk_TotalLabel, /*jproduk_bayar_cfField*/ jproduk_TotalBayarLabel, /*jproduk_hutang_cfField,*/jproduk_HutangLabel,jproduk_kembalianLabel, jproduk_pesanLabel, jproduk_lunasLabel] 
			}
			]
	});
		...
		items:[
			{
				...
				layout: 'form',
				border:false,
				items: [jproduk_nobuktiField, jproduk_custField, 
						{
							columnWidth:0.5,
							layout: 'column',
							border:false,
							items:[jproduk_cust_nomemberLabel, jproduk_cust_nomemberField,jproduk_valid_memberLabel, jproduk_valid_memberField]
	...
	combo_jual_produk.on('select',function(){
		...
		var today_s = new Date().format('d-m-Y');
		...
		if(editable_harga_produk == 1){
			...
			//alert(jproduk_cust_ultahField.getValue().format('Y-m-d'));
			var post_ultah = jproduk_cust_ultahField.getValue().add(Date.DAY, +7).format('m-d');
			var pre_ultah = jproduk_cust_ultahField.getValue().add(Date.DAY, -7).format('m-d');
			}

			//* Check no_member JIKA <>"" ==> jenis-diskon=DM /
			
			if((jproduk_cust_nomemberField.getValue()!=="") && (jproduk_valid_memberField.getValue().format('d-m-Y') >= today_s)){
				alert(today_s);
				if(pre_ultah <= today_ultah && post_ultah >= today_ultah){
	...
	*/
	...
	function load_total_bayar_updating(){
		...
		jproduk_kembalianLabel.setValue(kembalian);

		if(total_bayar>update_total_field){
			// jproduk_pesanLabel.setText("Kelebihan Jumlah Bayar");
			jproduk_kembalianLabel.setValue(CurrencyFormatted(kembalian));
			jproduk_kembalianField.setValue(kembalian);
		}else if(total_bayar<update_total_field || total_bayar==update_total_field){
			jproduk_pesanLabel.setText("");
			jproduk_kembalianLabel.setValue(0);
		}
		if(total_bayar==update_total_field){
	...
	master_jual_produk_createForm = new Ext.FormPanel({
		...
		width: 	1220,	//940,
		frame: true,
		items: [master_jual_produk_masterGroup, /*jproduk_groomingGroup,*/jproduk_custBaruGroup, detail_jual_produkListEditorGrid,master_jual_produk_bayarGroup]
		,
		buttons: [
			{
	...
	function jproduk_btn_cancel(){
		...
		jproduk_lunasLabel.setText('');
		jproduk_cust_priorityLabel.setText('');
		// jproduk_custBaruGroup.reset();
		// jproduk_custBaruGroup.setDisabled(false);
		// jproduk_custBaruGroup.collapse(true);
		master_jual_produk_custBaruGroup_reset();
	}
