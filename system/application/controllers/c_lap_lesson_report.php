...
class C_lap_lesson_report extends Controller {

	//constructor
	function C_lap_lesson_report(){
		parent::Controller();
		session_start();
		$this->load->model('m_lap_lesson_report', '', TRUE);	
	}
	
	//set index
	function index(){
		$this->load->helper('asset');
		$this->load->view('main/v_lap_lesson_report');
	}
	...
	function print_laporan(){
		...
		$data["data_print"]=$this->m_lap_lesson_report->get_laporan($tgl_awal,$bulan,$tahun,$tgl_akhir,$periode,$opsi,$customer,$lunas);
		$data["cust"]=$customer;
	...
	function  detail_fpiutang_bylp_list(){
		...
		$result=$this->m_lap_lesson_report->detail_fpiutang_bylp_list($fpiutang_nobukti,$query,$start,$end);
		echo $result;
	}
	//end of handler
	...
	function detail_detail_lunas_piutang_purge(){
		...
		$result=$this->m_lap_lesson_report->detail_detail_lunas_piutang_purge($master_id);
		echo $result;
	}
	//eof
	...
	function get_master_id(){
		$result=$this->m_lap_lesson_report->get_master_id();
		echo $result;
	}
	//
	...
	function get_fjual_bycust_list(){
		...
		if($task=='detail')
			$result=$this->m_lap_lesson_report->get_faktur_piutang_detail_list($cust_id,$query,$start,$end);
		/*elseif($task=='list')
			$result=$this->m_lap_lesson_report->get_faktur_piutang_all_list($cust_id,$query,$start,$end);*/
		elseif($task=='selected')
			$result=$this->m_lap_lesson_report->get_faktur_piutang_selected_list($fpiutang_id,$query,$start,$end);
		echo $result;
	}
	...
	function master_lunas_piutang_list(){
		...
		$end = (integer) (isset($_POST['limit']) ? @$_POST['limit'] : @$_GET['limit']);
		$task = isset($_POST['task']) ? @$_POST['task'] : @$_GET['task'];
		$result=$this->m_lap_lesson_report->master_lunas_piutang_list($query,$start,$end);
		echo $result;
	}
	...
	function master_lunas_piutang_update(){
		...
		$result = $this->m_lap_lesson_report->master_lunas_piutang_update($fpiutang_id ,$fpiutang_no ,$fpiutang_cust ,$fpiutang_tanggal, $fpiutang_keterangan ,$fpiutang_status
	...
	function master_lunas_piutang_create(){
		...
		$cetak_lp = trim(@$_POST["cetak_lp"]);
		$result=$this->m_lap_lesson_report->master_lunas_piutang_create($fpiutang_cust ,$fpiutang_tanggal, $fpiutang_keterangan ,$fpiutang_status
										,$fpiutang_cara ,$fpiutang_bayar
										,$fpiutang_kwitansi_no ,$fpiutang_kwitansi_nama
										,$fpiutang_card_nama ,$fpiutang_card_edc ,$fpiutang_card_no
										,$fpiutang_cek_nama ,$fpiutang_cek_no ,$fpiutang_cek_valid ,$fpiutang_cek_bank
	...
	function master_lunas_piutang_delete(){
		...
		$result=$this->m_lap_lesson_report->master_lunas_piutang_delete($pkid);
		echo $result;
	}
	...
	function print_paper(){
		...
		$result = $this->m_lap_lesson_report->print_paper($fpiutang_id);
		$rs=$result->row();
		$detail_fpiutang=$result->result();
		
		$cara_bayar=$this->m_lap_lesson_report->cara_bayar($fpiutang_id);
	...
	function master_lunas_piutang_batal(){
		$fpiutang_id=trim($_POST["fpiutang_id"]);
		$fpiutang_tanggal=trim(@$_POST["fpiutang_tanggal"]);
		$result=$this->m_lap_lesson_report->master_lunas_piutang_batal($fpiutang_id, $fpiutang_tanggal);
		echo $result;
	}
