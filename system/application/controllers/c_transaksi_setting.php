...
class C_transaksi_setting extends Controller {
	...
	function transaksi_setting_update(){
		...
		$kembali_poin_days=trim(@$_POST["kembali_poin_days"]);
		$uang_pangkal_enrichment=trim(@$_POST["uang_pangkal_enrichment"]);
		$uang_sekolah_enrichment=trim(@$_POST["uang_sekolah_enrichment"]);
		$trans_update=@$_SESSION[SESSION_USERID];
		$trans_date_update=date(LONG_FORMATDATE);
		$result = $this->m_transaksi_setting->transaksi_setting_update($trans_op_days, $mb_days, $kembali_poin_days, $uang_pangkal_enrichment, $uang_sekolah_enrichment, $trans_update, $trans_date_update);
		echo $result;
	}
