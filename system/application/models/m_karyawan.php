...
class M_karyawan extends Model{
        ...
        function detail_jabatan_delete($kjabatan_id){
            $date_now = date('Y-m-d');
			$datetime_now = date('Y-m-d H:i:s');
		...
		//function for detail
		//get record list
		function detail_gaji_list($master_id,$query,$start,$end) {
			$query = "SELECT * FROM karyawan_gaji where kgaji_master='".$master_id."' ORDER BY kgaji_tanggal ASC";
		...
		//insert detail record
		function detail_gaji_insert($array_kgaji_id , $kgaji_master, $array_kgaji_tanggal, $array_kgaji_nominal, $array_kgaji_keterangan){
			//$datetime_now = date('Y-m-d H:i:s');
			//if master id not capture from view then capture it from max pk from master table
			if($kgaji_master=="" || $kgaji_master==NULL || $kgaji_master==0){
				$kgaji_master=$this->get_master_id();
			}
			
			$size_array = sizeof($array_kgaji_nominal) - 1;
			for($i = 0; $i < sizeof($array_kgaji_nominal); $i++){
				$kgaji_id = $array_kgaji_id[$i];
				$kgaji_master = $kgaji_master;
				$kgaji_tanggal = $array_kgaji_tanggal[$i];
				$kgaji_nominal = $array_kgaji_nominal[$i];
				$kgaji_keterangan = $array_kgaji_keterangan[$i];

				$sql = "SELECT kgaji_id
						,kgaji_revised
					FROM karyawan_gaji
					WHERE kgaji_id='".$kgaji_id."'";
				$rs = $this->db->query($sql);
				...
				// jika datanya sudah ada maka update saja
					$dtu_kgaji = array(
						"kgaji_master"=>$kgaji_master,
						"kgaji_tanggal"=>$kgaji_tanggal,
						"kgaji_nominal"=>$kgaji_nominal,
						"kgaji_keterangan"=>$kgaji_keterangan
					);
					$this->db->where('kgaji_id', $kgaji_id);
					$this->db->update('karyawan_gaji', $dtu_kgaji); 
					if($this->db->affected_rows()){
						$sql="UPDATE karyawan_gaji set kgaji_revised=(kgaji_revised+1) WHERE kgaji_id='".$kgaji_id."'";
						$this->db->query($sql);
					}
				} else {
					$data = array(
					...
					$this->db->insert('karyawan_gaji', $data); 
		...
		//Delete detail_gaji
        function detail_gaji_delete($kcuti_id){
            $date_now = date('Y-m-d');
			$datetime_now = date('Y-m-d H:i:s');
			$query = "DELETE FROM karyawan_gaji WHERE kgaji_id = ".$kgaji_id;
			$this->db->query($query);
			if($this->db->affected_rows()>0)
				return '1';
			else
		...
		function karyawan_list($filter,$start,$end){
			...
			$query =   "SELECT 
							vu_karyawan.*, departemen.*, cabang.*, jabatan.jabatan_nama, bank_master.mbank_nama as karyawan_bank_nama, golongan.* 
						FROM vu_karyawan
			...
			$query .= " ORDER BY karyawan_date_create DESC";
		...
		if($karyawan_cab_sjl=='true')
			$sjl="1";
		if($karyawan_cab_sjl=='false')
			$sjl="0";	
			
		if($karyawan_cab_smg=='true')
			$smg="1";
		if($karyawan_cab_smg=='false')
			$smg="0";	
			
		$temp_cabang=$th.$ki.$hr.$tp.$dps.$jkt.$mta.$blpn.$kuta.$corp.$mks.$mdn.$lbk.$mnd.$ygk.$mlg.$btm.$maa.$mg.$sjl.$smg;
