...
class M_appointment extends Model{
	...
	function get_dokter_list($query, $tgl_app, $karyawan_jabatan){
		...
		$sql=  "SELECT 
					karyawan_id,karyawan_no,karyawan_nama,karyawan_username
				FROM karyawan
				LEFT JOIN karyawan_jabatan on (karyawan_jabatan.kjabatan_master = karyawan.karyawan_id)
				WHERE karyawan_jabatan.kjabatan_jabatan = 7
				";
		if($query<>""){
	...
	//Ambil Services berdasarkan kategori BABYSPA
	function get_rawat_medis_list($query,$start,$end){
		...
		// $sql="SELECT * FROM vu_perawatan WHERE (kategori_nama='Medis' OR kategori_nama='Surgery' OR kategori_nama='Anti Aging') AND rawat_aktif='Aktif'";
		$sql="SELECT * FROM perawatan 
			LEFT JOIN kategori on (perawatan.rawat_kategori = kategori.kategori_id)
		WHERE (kategori.kategori_nama='Swim and Spa') AND perawatan.rawat_aktif='Aktif'";
		if($query<>"" && is_numeric($query)==false){
			$sql.=" and (perawatan.rawat_kode like '%".$query."%' or perawatan.rawat_nama like '%".$query."%')";
		}else{
			if($rs_rows){
				$filter="";
				...
				foreach($rs->result() as $row_dapp){
					...
					$filter.="OR perawatan.rawat_id='".$row_dapp->dapp_perawatan."' ";
				}
				$sql=$sql."(".substr($filter,2,strlen($filter)).")";
			}
		}
		$sql.=" ORDER BY perawatan.rawat_nama ASC";
	   ...
	   */
		...
		if($rs_trawat->num_rows()){
			...
			*/
			...
			if($kategori_nama=='Baby Spa' || $kategori_nama=='Surgery' || $kategori_nama=='Anti Aging'){
				$dti_dtrawat = array(
					"dtrawat_master"=>$dtrawat_master,
					"dtrawat_dapp"=>$dapp_id,
	...
	/*
	function appointment_list($filter,$start,$end,$tgl_app,$jam_awal,$jam_akhir,$jenis_rawat,$dokter_id){
		if($jam_awal!=''){		
			$jam_awal=$jam_awal.':00';
		}
		if($jam_akhir!=''){	
			$jam_akhir=$jam_akhir.':00';
		}
		
		if($jenis_rawat=="")
			$jenis_rawat="Baby Spa";
		//$query = "SELECT * FROM appointment";
		$dt=date('Y-m-d');
		$dt_six=date('Y-m-d',mktime(0,0,0,date("m"),date("d")+6,date("Y")));
		$query="SELECT * FROM vu_appointment";
		
		if($jenis_rawat=="Baby Spa" && $filter==''){
			$query .=eregi("WHERE",$query)? " AND ":" WHERE ";
			$query .=" (kategori_nama='Baby Spa')";
			if($tgl_app!=""){
				$tgl_app=date('Y-m-d', strtotime($tgl_app));
			...
			if($jam_awal!='' or $jam_akhir!=''){
				$query.=eregi("WHERE",$query)?" AND ":" WHERE ";
				if($jam_awal!='' and $jam_akhir!=''){
					$query.= " dapp_jamreservasi >= '".$jam_awal."' AND dapp_jamreservasi <= '".$jam_akhir."'";
				}else if ($jam_awal!='' and $jam_akhir==''){
					$query.= " dapp_jamreservasi >= '".$jam_awal."'";
				}else if ($jam_awal=='' and $jam_akhir!=''){
					$query.= " dapp_jamreservasi <= '".$jam_akhir."'";
				}
			}
			// For simple search
		...
		if ($dokter_id<>"" && is_numeric($dokter_id)==true){
			...
			$query .=eregi("WHERE",$query)? " AND ":" WHERE ";
			//search customer,perawatan,dokter,therapist
			$query .= " (dokter_id = '".addslashes($dokter_id)."') AND (kategori_nama='Baby Spa')";
 
		}
		
		// For Pilihan Tanggal Appointment di tbar dengan pilihan dokter kosong
		if($dokter_id=="" && is_numeric($dokter_id)==false && $tgl_app!="" && $jenis_rawat=="Baby Spa"){
			$dt=date('Y-m-d', strtotime($tgl_app));
			$query="SELECT * FROM vu_appointment WHERE (kategori_nama='Baby Spa') AND dapp_tglreservasi = '$dt'";
	...
	*/
	...
	//function for get list record
	function appointment_list($filter,$start,$end,$tgl_app,$jam_awal,$jam_akhir,$jenis_rawat,$dokter_id){
		$query = "SELECT appointment.app_id as app_id, appointment.app_customer as app_customer, appointment.app_tanggal as app_tanggal, appointment.app_cara as app_cara, appointment.app_keterangan as app_keterangan, appointment.app_creator as app_creator, appointment.app_date_create as app_date_create,
			appointment_detail.dapp_jamreservasi as dapp_jamreservasi, appointment_detail.dapp_tglreservasi as dapp_tglreservasi, appointment_detail.dapp_jambatal as dapp_jambatal, appointment_detail.dapp_jamkonfirmasi as dapp_jamkonfirmasi, appointment_detail.dapp_id as dapp_id, appointment_detail.dapp_status as dapp_status,
			customer.cust_nama as cust_nama, customer.cust_id as cust_id, customer.cust_no as cust_no,
			perawatan.rawat_id as rawat_id , perawatan.rawat_nama as rawat_nama,
			karyawan.karyawan_nama as terapis_nama, karyawan.karyawan_id as terapis_id, karyawan.karyawan_username as terapis_username

					FROM appointment
					JOIN appointment_detail on (appointment_detail.dapp_master = appointment.app_id)
					JOIN perawatan on (appointment_detail.dapp_perawatan = perawatan.rawat_id)
					JOIN customer on (customer.cust_id = appointment.app_customer)
					JOIN karyawan on (karyawan.karyawan_id = appointment_detail.dapp_petugas)
					";
		...
		if ($filter<>""){
			...
			$query .= " (cust_nama LIKE '%".addslashes($filter)."%' OR 
						 app_cara LIKE '%".addslashes($filter)."%' OR 
						 app_tanggal LIKE '%".addslashes($filter)."%' )";
		}
	...
	function appointment_update($app_id
		...
		$sql="SELECT app_cara,app_keterangan,app_revised, app_customer, app_tanggal FROM appointment WHERE app_id='$app_id'";
		$rs=$this->db->query($sql);
		if($rs->num_rows()){
			$rs_record=$rs->row_array();
			if($rs_record["app_cara"]!=$app_cara || $rs_record["app_keterangan"]!=$app_keterangan || $rs_record["app_tanggal"]!=$app_tanggal || is_numeric($app_customer)){ /* JIKA terjadi PERUBAHAN */
				$data = array(
					"app_cara"=>$app_cara, 
		...
		*/
		...
		if($rs->num_rows()){
			...
			*/
			...
			}else if(($kategori_nama=='Baby Spa' || $kategori_nama =='Surgery' || $kategori_nama =='Anti Aging') && is_numeric($dokter)==true && $dokter_awal<>$dokter){
...
*/
	...
	function appointment_print($app_customer
		...
		if($option=='LIST'){
			...
			if($jenis_rawat=="Baby Spa" && $filter==''){
				$query .=eregi("WHERE",$query)? " AND ":" WHERE ";
				$query .=" (kategori_nama='Baby Spa' OR kategori_nama = 'Surgery' OR kategori_nama = 'Anti Aging')";
				if($tgl_app!=""){
					$tgl_app=date('Y-m-d', strtotime($tgl_app));
					$query .=" AND dapp_tglreservasi='$tgl_app'";
				}else{
			...
			if ($filter<>"" && is_numeric($filter)==true){
				...
				$query .=eregi("WHERE",$query)? " AND ":" WHERE ";
				//search customer,perawatan,dokter,therapist
				$query .= " (dokter_id = '".addslashes($filter)."') AND (kategori_nama='Baby Spa' OR kategori_nama = 'Surgery' OR kategori_nama = 'Anti Aging')";
			...
			// For Pilihan Tanggal Appointment di tbar dengan pilihan dokter kosong
			if($filter=="" && is_numeric($filter)==false && $tgl_app!="" && $jenis_rawat=="Baby Spa"){
				$dt=date('Y-m-d', strtotime($tgl_app));
				$query="SELECT * FROM vu_appointment WHERE (kategori_nama='Baby Spa' OR kategori_nama ='Surgery' OR kategori_nama = 'Anti Aging') AND dapp_tglreservasi = '$dt'";
