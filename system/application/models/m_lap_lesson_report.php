...
class M_lap_lesson_report extends Model{
		
	//constructor
	function M_lap_lesson_report() {
		parent::Model();
	}
	...
	function get_laporan($tgl_awal,$bulan,$tahun,$tgl_akhir,$periode,$opsi,$customer,$lunas){
		...
		if($opsi=='rekap'){
			...
			$sql="
			SELECT * FROM detail_lesson_plan
			LEFT JOIN master_lesson_plan ON master_lesson_plan.lesplan_id = detail_lesson_plan.dplan_master
			WHERE master_lesson_plan.lesplan_month='".$bulan."'
			AND master_lesson_plan.lesplan_year='".$tahun."'";
	...
	function master_lunas_piutang_update($fpiutang_id ,$fpiutang_no ,$fpiutang_cust ,$fpiutang_tanggal, $fpiutang_keterangan ,$fpiutang_status
		...
		if($rs>-1){
			...
			$rs_dpiutang_insert = $this->detail_lunas_piutang_insert($fpiutang_cust ,$array_dpiutang_id ,$array_lpiutang_id ,$fpiutang_no ,$array_dpiutang_nilai ,$fpiutang_cara, $array_dpiutang_keterangan ,$cetak_lp);
